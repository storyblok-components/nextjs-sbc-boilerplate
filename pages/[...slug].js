...
import {getAllLanguageCodes, getAllPublishedStoriesSlugs} from '../lib/storyblok-gapi'
...
export async function getStaticProps(context) {
    const {Space: {languageCodes}} = await getAllLanguageCodes()
    ...
    try {
        const { data: { story } } = await Storyblok.get(completeStoryblokDataUrl, {
...
export async function getStaticPaths() {
    const {PageItems: {items}} = await getAllPublishedStoriesSlugs()
    const {Space: {languageCodes}} = await getAllLanguageCodes()
    ...
    const paths = [
        ...pathsWithLanguageCode,
        ...pathsWithoutLanguageCode
    ]
